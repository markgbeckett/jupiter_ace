...
: RANGE ( N -- N )
    ( CONVERT DISTANCE INTO BITMAP FOR DIGITAL IO BOARD )
    DUP 7 > IF ( THEN NO LEDS LIT )
	DROP 0
    ELSE
	( OTHERWISE, WORK OUT 2^[8-D]-1 )
	8 SWAP -
	2 SWAP POWER
	1-
    THEN
...
: TEST
	...
	REXVIS @ 1+ DUP IF
	    ...
	    DUP RANGE DIOPORT OUT ( UPDATE DIGITAL IO BOARD )
	    
	    [ 3DVIEW 9 + ] LITERAL CALL ( DRAW REX )
