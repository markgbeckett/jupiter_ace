...
For example, if MOVE expands the maze one cell to the right, then in the new cell, I record that the generator came from the left. Then, if the new cell turns out to be a deadend, I can look up from which direction the generator came and backtrack (to the left).
